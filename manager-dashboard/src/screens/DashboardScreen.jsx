...
export default function DashboardScreen({
  ...
  async function handleAccountUpdate(event) {
    ...
    if (accountPassword) {
      updates.password = accountPassword;
    }

    if (!updates.email && !updates.password) {
      setAccountMessage('Enter a new email or password to update your account.');
    ...
    setAccountMessage(
      updates.email
        ? 'Account update saved. Check the new email address for a confirmation link.'
        : 'Password updated.'
    );
    onRefresh();
    setAccountBusy(false);
      ...
      {isAccountDialogOpen ? (
        <div className="modal-backdrop" role="presentation">
          ...
          >
            ...
            <h3 id="account-edit-title">Edit account</h3>
            <p>Change your email or password. Your name stays managed by the admin account list.</p>
            ...
            <div className="confirm-dialog-actions">
              ...
              <button type="submit" disabled={accountBusy}>
                {accountBusy ? <Loader2 className="spin" size={16} /> : <UserCog size={16} />}
                Save changes
              </button>
            </div>
          </form>
