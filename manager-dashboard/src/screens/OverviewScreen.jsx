...
export function buildOperationAlerts(dashboard) {
  ...
  if (dashboard?.pendingApprovals?.length) {
    alerts.push({
      ...
      severity: 'info',
      title: 'Account approvals waiting',
      detail: `${dashboard.pendingApprovals.length} account request(s) need admin review.`,
    });
  }
    ...
    <section className="pending-approval-notice">
      <div>
        ...
        <div>
          ...
          <p>{pendingCount} account request(s) need your review.</p>
...
function LiveSummaryPanel({ dashboard, panelRef }) {
  ...
  const summaryItems = [
    ...
    { label: 'Pending', value: stats.pendingAccounts ?? stats.pendingOperators ?? 0, icon: Clock },
